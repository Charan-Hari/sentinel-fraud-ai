...
const mixedScenario = "public/demo-data/mixed-review-queue.csv";
...
try {
  ...
  await page.locator('input[type="file"]').setInputFiles(mixedScenario);
  await page.getByText(/transactions validated/i).waitFor();
  await page.getByRole("button", { name: /Score uploaded transactions/i }).click();
  ...
  await page.getByRole("button", { name: /Generate AI brief/i }).click();
  ...
  if (videoPath) {
    copyFileSync(videoPath, `${videoDir}/sentinel-demo.webm`);
  }
  await context.close();
  await browser.close();
}

console.log("Sentinel screenshots and video created.");
